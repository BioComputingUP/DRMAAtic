...
class JobFilterSet(FilterSet):
    job_name = CharFilter(field_name='job_name__name', lookup_expr='icontains')
    description = CharFilter(field_name='_job_description', lookup_expr='icontains')
    status = ChoiceFilter(choices=Job.Status.choices, field_name="_status", lookup_expr='icontains')
