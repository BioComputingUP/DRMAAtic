...
from submission.utils import get_ip, get_params, create_job_folder, format_job_params
...
class JobSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if "task" not in validated_data.keys():
            raise exceptions.NotAcceptable("The 'task' parameter needs to be specified")

        task = Task.objects.get(name=validated_data["task"])
        ...
        try:
            job_params, renamed_files = get_params(self.initial_data, job, parameters_of_job)
